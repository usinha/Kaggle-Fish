...
#
def extractFishImages(model, fullImagePath) :
	image = cv2.imread(fullImagePath)
	imagename = fullImagePath
	OUT_DIRECTORY = "DerivedImages"
	TOP_SIZE = 3 # size of top images based on probality it's a fish
	...
	for resized in pyramid(image, scale=1.5,minSize=(128, 128)):
		...
		for (x, y, window) in sliding_window(resized, stepSize=32, windowSize=(winW, winH)):
			...
			if fishFound :
				...
				tup = (pr, window, fullOutFileName)
				topItems.append(tup)
				print ("appended image")
				if len(topItems) > TOP_SIZE :
					topItems = sorted(topItems,key = lambda t:t[0], reverse= True)[0:TOP_SIZE]
	##
	for fish in topItems:
		print ("output .. " + fish[2])
...
if __name__ == "__main__" :
	...
	for file in os.listdir(imagedir):
		if file.endswith(".jpg") :
			extractFishImages(model,imagedir + "/" + file)
