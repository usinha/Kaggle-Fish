...
OUT_DIRECTORY = '/home/icarus/kaggle/Kaggle-Fish/data/Final_Derived_Images_bb' #"/home/pyimagesearch/kaggle/FINAL_DerivedImages"
...
def extractFishImages(VGG_model,model, model_3,imagedir,filenm) :
        ...
        for (x, y, window) in sliding_window(resized, stepSize=32, windowSize=(winW, winH)):
            ...
            if VGG_fishFound :
                ...
                if fishFound :
                    ...
                    if fishFound:
                        filenmInd2 = filenm.find(".jpg")
                        filenm_only = filenm[:filenmInd2]
                        out_file_name = filenm_only  \
                            + "-" + str(py_num) \
                            + "-" + str(win_num) + "-" + str(num_matches) \
                            + "p" + "{0:.2f}".format(VGG_pr) \
                            + "p" + "{0:.2f}".format(pr) \
                            + "p" + "{0:.2f}".format(pr_3) \
                            + ".jpg"
                        fullOutFileName = os.path.join(OUT_DIRECTORY,out_file_name)
    ...
    for t in topItems:
        for fish in t:
            #total_out_images += 1
            final_images.append(fish)
    # sort the list
    final_images =  sorted(final_images,key = lambda t:t[0], reverse= True)[0:FINAL_TOP_SIZE]
